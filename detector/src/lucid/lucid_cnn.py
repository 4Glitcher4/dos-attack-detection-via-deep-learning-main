...
OUTPUT_FOLDER = f".{os.sep}output{os.sep}"
...
def main(argv):
    ...
    if args.train is not None:
        subfolders = glob.glob(args.train[0] + f"{os.sep}*{os.sep}")
        if (
            len(subfolders) == 0
        ):  # for the case in which the is only one folder, and this folder is args.dataset_folder[0]
            subfolders = [args.train[0] + os.sep]
        else:
            subfolders = sorted(subfolders)
        for full_path in subfolders:
            full_path = full_path.replace(
                os.sep * 2, os.sep
            )  # remove double slashes when needed
            folder = full_path.split(os.sep)[-2]
            dataset_folder = full_path
            print
            print(glob.glob(dataset_folder + f"{os.sep}*" + "-train.hdf5"))
            print(glob.glob("./*"))
            print(f"{dataset_folder}{os.sep}*-train.hdf5".replace(os.sep * 2, os.sep))
            X_train, Y_train = load_dataset(  # remove double slashes when needed
                f"{dataset_folder}{os.sep}*-train.hdf5".replace(os.sep * 2, os.sep)
            )
            X_val, Y_val = load_dataset(dataset_folder + f"{os.sep}*" + "-val.hdf5")

            X_train, Y_train = shuffle(X_train, Y_train, random_state=SEED)
            X_val, Y_val = shuffle(X_val, Y_val, random_state=SEED)

            # get the time_window and the flow_len from the filename
            train_file = glob.glob(dataset_folder + f"{os.sep}*" + "-train.hdf5")[0]
            filename = train_file.split(os.sep)[-1].strip()
            ...
            row = {
                ...
                "Validation Set": glob.glob(
                    dataset_folder + f"{os.sep}*" + "-val.hdf5"
                )[0],
            }
    ...
    if args.predict is not None:
        ...
        dataset_filelist = glob.glob(args.predict + f"{os.sep}*test.hdf5")

        if args.model is not None:
            model_list = [args.model]
        else:
            model_list = glob.glob(args.predict + f"{os.sep}*.h5")

        for model_path in model_list:
            model_filename = model_path.split(os.sep)[-1].strip()
            filename_prefix = (
    ...
    if args.predict_live is not None:
        ...
        if args.predict_live is None:
            ...
            data_source = pcap_file.split(os.sep)[-1].strip()
        else:
